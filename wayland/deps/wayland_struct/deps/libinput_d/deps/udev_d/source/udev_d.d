...
udev* udev_new();
void udev_set_log_fn(udev* udev, void function(udev* udev, int priority,
        const(char)* file, int line, const(char)* fn, const(char)* format, va_list args) log_fn);
...
int udev_device_set_sysattr_value(udev_device* udev_device,
...
int udev_enumerate_add_match_sysattr(udev_enumerate* udev_enumerate,
        const(char)* sysattr, const(char)* value);
int udev_enumerate_add_nomatch_sysattr(udev_enumerate* udev_enumerate,
        const(char)* sysattr, const(char)* value);
int udev_enumerate_add_match_property(udev_enumerate* udev_enumerate,
...
int udev_queue_get_seqnum_sequence_is_finished(udev_queue* udev_queue, ulong start, ulong end);
